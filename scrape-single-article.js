...
async function main() {
  try {
    ...
    // 只处理一篇文章，从命令行参数获取索引
    const articleIndex = process.argv[2] ? parseInt(process.argv[2]) : 0;
    if (articleIndex >= 0 && articleIndex < articles.length) {
      const article = articles[articleIndex];
      console.log(`Scraping article ${articleIndex + 1}/${articles.length}: ${article.title}`);
      ...
      console.error(`Invalid article index: ${articleIndex}. Must be between 0 and ${articles.length - 1}`);
    }
